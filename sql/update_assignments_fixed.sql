...
-- Add new status column and rename old one
ALTER TABLE assignments 
ADD COLUMN status_assignment ENUM('ditugaskan', 'diproses', 'selesai', 'terlambat', 'dibatalkan') NOT NULL DEFAULT 'ditugaskan' AFTER catatan;

-- Update data to new status format
UPDATE assignments SET status_assignment = CASE 
    WHEN status = 'assigned' THEN 'ditugaskan'
...
ADD INDEX idx_status_assignment (status_assignment),
...
UPDATE assignments SET 
    judul_assignment = COALESCE(role_assignment, 'Assignment Default'),
...
-- Insert additional sample assignments
